...
function getNextFriday(date) {
  ...
  let newDay = day + (friday - (weekday === 6 ? -1 : weekday));
  const res = new Date(
    date.setDate(newDay === day ? (newDay = day + 7) : newDay)
  );
  return res;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const periodTime = end.getTime() - start.getTime();
  return periodTime / (1000 * 3600 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return endTime >= dateTime && dateTime >= startTime;
}
...
function getWeekNumberByDate(date) {
  ...
  const days = Math.floor((date - firstDay) / (1000 * 3600 * 24));
...
function getNextFridayThe13th(date) {
  ...
  let friday = new Date(date.getFullYear(), month, 13);

  while (friday.getDay() !== 5) {
    month += 1;
    friday = new Date(date.getFullYear(), month, 13);
  }
...
function isLeapYear(date) {
  ...
  return year % 4 === 0 ? true : year % 100 === 0 && year % 400 === 0;
}
